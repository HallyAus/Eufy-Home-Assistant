...
// Two frame managers: a SENDER (mode 1) that turns app frames into PTCS wire packets, and a
// RECEIVER (mode 0) that turns received PTCS packets back into whole frames.
const fs = require("fs");
...
// Eufy sometimes returns control failures as a signed int32 followed only by zeros.
// -104 is reproducibly returned when a shared/member user reaches an NVR command session.
function fixedControlStatus(frame) {
  if (!Buffer.isBuffer(frame) || frame.length < 20 || frame.toString("ascii", 0, 4) !== "XZYH") return null;
  if (frame.readUInt16LE(4) === 1032) return null; // per-channel status uses an int32 legitimately
